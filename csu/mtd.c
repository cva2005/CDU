...
if ((Method_ARD[method_cnt]==0)&&(method_cnt>0)) Method_ARD[method_cnt]=Method_ARD[method_cnt-1]+SIZE_METHOD+(SIZE_STAGE*Method.fld.Nstage);

cnt=EEPROM_read_string(Method_ARD[method_cnt], SIZE_METHOD, &Method.byte[0]);//прочитать метод из памяти
if ((!cnt)||(Method.fld.data_type!=0xAA)) //проверить корректность метода
	{ 
	if (method_cnt>method_def) method_cnt=0;  //если не причтался не заводской метод, то установить на первый заводской метод
	if (!EEPROM_read_string(Method_ARD[method_cnt], SIZE_METHOD, &Method.byte[0])) create_method(method_cnt);	//если не прочитался заводской метод, то создать его*/
	}

stage_cnt=0;
read_stage(stage_cnt);
calculate_method();
cycle_cnt=0;    //установить первый цикл
}

void read_stage(unsigned char stage)
{
if (!EEPROM_read_string(Method_ARD[method_cnt]+SIZE_METHOD+(SIZE_STAGE*stage), SIZE_STAGE, &Stage.byte[0]))	Error=ERR_STAGE;
...
while (ADR<EEPROM_SIZE)
	...
	//RS485_Tx; Putch(ADR,1);
	//_delay_ms(10);
	//RS485_Tx; Putch(MTD.fld.data_type,1);
	//_delay_ms(1000);
	...
	// return (ADR);//проверить корректность метода, если метод неверный начит эта область памяти свободна
	}
//_delay_ms(10000);
return (ADR);
}
