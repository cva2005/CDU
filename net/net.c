...
void init_rs(void)
{
    RS_DIR_INIT();
    SET_BAUD(115200);
    UART(UCSR,C) = SHL(UART(UCSZ,1)) | SHL(UART(UCSZ,0)); // Data Bits: 8
...
void usart_rx_byte(void)
{
    ...
    if (Tmp & (SHL(UART(FE,)) | SHL(UART(DOR,)))) { /* апп. ошибка FERR/OERR */
        ...
        ASCII_TMR_ON(); /* запустить таймер тайм-аута приема кадра ASCII */
        RTU_TMR_ON(); /* запустить таймер тайм-аута приема кадра RTU */
 ...
 * при преме кадра MODBUS ASCII, KRON.
 */
#pragma vector=RS_OCRnX_IRQ(ASCII)
#pragma type_attribute=__interrupt
void rx_ascii_tick_irq(void)
{
    AsciiIdleCount++; /* скорректировать интервал "молчания" */
    if (AsciiIdleCount == FRAME_ASCII_ERROR) { /* межсимв. интервал превышен */
        ASCII_TMR_OFF(); /* остановить таймер */
        if (AsciiBusState != BUS_STOP) { /* активен прием кадра MODBUS ASCII */
            AsciiBusState = BUS_IDLE; /* шину в режим ожидания */
        }
        if (KronBusState != BUS_STOP) { /* активен прием кадра DCON */
 ...
 */
#pragma vector=RTU_OCR_IRQ
#pragma type_attribute=__interrupt
void rx_tick_irq(void)
{
    RtuIdleCount++; /* скорректировать интервал "молчания" */
    if (RtuIdleCount == FRAME_RTU_SYNC) { /* межкадровый интервал выдержан */
        RTU_TMR_OFF(); /* остановить таймер */
