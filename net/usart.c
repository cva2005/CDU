...
if (rx_point>4) //если принято больше 4-х байт
	...
	if (rx_pack.fld.header.length<=(rx_point-5)) //если число принятых байт данных равно значению в поле длинны и принято CRC
		...
		if ((rx_pack.fld.header.dest_adr!=MY_ADR)&&(rx_pack.fld.header.dest_adr!=SYS_ADR)&&(rx_pack.fld.header.dest_adr!=BROAD_ADR))  //Если на совпадает адрес получателя, то проверить мультикастовый пакет или нет
			{
